...
- `intent/` 简短说明为什么建设这个系统、希望它成为什么，以及已经确定的项目边界；可以记录长期动机和与 Current 能力明确区分的方向。
...
初始 intent 只有一篇简短的 `README`。它从项目作者的视角回答为什么建设、希望成为什么，以及项目边界是什么。长期动机和尚未实现的方向可以进入 intent，但必须与 Current 能力明确区分；无需为了填满通用模板而强行补齐受众、取舍或重新评估条件。
