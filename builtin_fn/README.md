...
All production declarations live together in `src/builtins.rs`, with exactly one
`builtin_functions!` invocation per category. The DSL lowers directly to
`BuiltinCategory`, catalog presentation metadata, `FunctionSig`, and `ParamShape`;
production code does not parse signature strings at runtime.
...
implementation obligations once that generator is introduced.

## Entry points

- `builtin_categories()` returns the complete ordered catalog, including unsupported
  declarations.
- `builtins_functions()` returns supported semantic signatures only.
