...
pub fn render_builtin_readme(
    ...
    let relative_end = current[content_start..]
        .find(CATALOG_END_MARKER)
        .ok_or(CatalogRegionError::MissingEndMarker)?;
    let end = content_start + relative_end + CATALOG_END_MARKER.len();
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingEndMarker => write!(
                formatter,
                "builtin README is missing `{CATALOG_END_MARKER}` after the begin marker"
            ),
        }
    }
}
