...
use builtin_fn::{FunctionSig, ParamRef, normalize_union, type_accepts};
...
pub(crate) fn convert_column<K: ColumnKind>(
    ...
    K::from_column(column).map_err(|column| PreparedArgumentError::WrongPhysicalKind {
        expected: K::ABI_KIND,
        actual: column.abi_kind(),
    })
}
