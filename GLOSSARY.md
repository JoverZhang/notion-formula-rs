...
| builtin declaration DSL | `builtin_functions!` | The domain-specific syntax embedded in Rust for declaring builtin categories, signatures, support markers, and related metadata. `builtin_fn_macros` parses, validates, and expands this Current DSL; proposed future DSLs are not Current features. | builtin catalog when referring to declaration syntax |
