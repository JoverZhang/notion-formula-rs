...
- Start at the knowledge base entry `docs/design/README.md` and follow the repo's docs structure from there.
- There could be multiple places as the request might include multiple objectives. For each objective:
  ...
  - Read through all categories, find out which category is the best fit.
    - If there is no obvious best answer, create a new category. A new category comes with a new guideline file; add it under `docs/`.
