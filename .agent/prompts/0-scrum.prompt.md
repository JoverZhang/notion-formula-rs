...
- Task logs: `.agent/tasklogs/` (this prompt edits `agent_scrum.md`)
  - `agent_scrum.md` should already exist (it may contain content from a previous scrum).
  - If you cannot find it, you are looking in the wrong folder.
- Knowledge base entry: `docs/design/README.md`

## Goal and Constraints

- Your goal is to finish a design document in `agent_scrum.md` to address a problem.
- You are only allowed to update `agent_scrum.md`.
...
- You should override `agent_scrum.md` with only one title `# !!!SCRUM!!!`.
  - At the moment, `agent_scrum.md` may contain old tasks from previous requests, even it may look like the document is already finished for the current scrum, always clean it up.
...
I am going to propose some change to `agent_scrum.md`.
...
- Your goal is to help me break down the problem into small tasks, write it down to `agent_scrum.md`.
- Each task should be:
...
- Ensure there is a `# !!!FINISHED!!!` mark at the end of `agent_scrum.md` to indicate the document reaches the end.
...
- The current `agent_task.md`, `agent_planning.md` and `agent_execution.md` are associated to that task.

### Step 6.2. Identify My Inputs

- Read through `agent_execution.md`. There may be some fixing attempts, that were done by you.
- Compare existing source code with `agent_execution.md`, finding what is changed.
  - Don't rely on `git` to identify changes, as I always commit them periodically. You need to compare the actual source code with `agent_execution.md`.
...
- If there is no `# !!!VERIFIED!!!` in `agent_execution.md`, it means you failed to deliver the task, either the code did not compile or some test cases failed. My edit will also reflect the final solution to the task.
...
- Create a new file `agent_execution_finding.md` with a topic `# Comparing to User Edit`.
  - `agent_execution_finding.md` should stay in the same folder as `agent_execution.md`.
- Add your finding to `agent_execution_finding.md`.

### Step 6.4 Learn

- There will be multiple `# UPDATES` or `# FIXING ATTEMPTS` or `# Comparing to User Edit` sections in `agent_task.md`, `agent_planning.md`, `agent_execution.md` and `agent_execution_finding.md`.
...
- Check all future tasks, apply what you have learned, and adjust your approach accordingly.
  - For each unfinished task that can be improved, update related learnings in `agent_scrum.md`.

### Step 6.5 Backup

- You must only run this step after finishing all above steps.
  - In the above steps, necessary change will be updated to `agent_execution.md` and `agent_scrum.md`.
- There is no backup script in this repo.
- Keep task logs under `.agent/tasklogs/` and follow `.agent/README.md` for how to mark `# !!!VERIFIED!!!` when `just verify` succeeds.
