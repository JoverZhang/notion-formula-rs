...
- Task logs: `.agent/tasklogs/` (this prompt edits `agent_kb.md`)
- Knowledge base entry: `docs/design/README.md`

## Goal and Constraints

- Your goal is to draft a document for the knowledge base in `agent_kb.md`.
- You are only allowed to update `agent_kb.md` and the knowledge base.
- You are not allowed to modify any other files.
- Code references must be wrapped in either `single-line` or ```multi-line``` quotes.
- Start at the knowledge base entry `docs/design/README.md` and understand how it is organized.
  - `KB entry` below means this file.
...
- If there is a `# Topic` section: it means you are on a fresh start.
  - You should override `agent_kb.md` with only one title `# !!!KNOWLEDGE BASE!!!`.
...
- If there is nothing: it means you are accidentally stopped. Please continue your work.
  - Read `agent_kb.md` thoroughly, it is highly possible that you were working on the request described in the last section in `# DOCUMENT REQUEST`.
...
- Your goal is to complete a draft document for the knowledge base in `agent_kb.md`, append the draft after the file.
...
- Make a `# DRAFT-LOCATION` section, you need to describe which part you would like to update in the knowledge base.
  ...
  - But at the moment only edit `agent_kb.md`, do not edit `Index.md`. You are drafting a document, it will be implemented in `Index.md` only after my approval.
